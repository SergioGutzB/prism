            ...
            maybe_event = event_rx.recv() => {
                ...
                match event {
                    ...
                    AppEvent::FixTaskFailed(idx, err) => {
                        if let Some(task) = app.fix_tasks.iter_mut().find(|t| t.index == idx) {
                            // Write the error into output so the panel shows it —
                            // the UI renders task.output, not task.status.
                            task.output = format!("❌ Error: {err}");
...
fn start_ai_fix(
    ...
    // Collect fixable inline comments:
    //   - New AI/manual comments that are not rejected (need approval before fixing)
    //   - Existing GitHub comments (already published — always fixable regardless of status)
    // Both require a file_path to produce a meaningful fix.
    let comments: Vec<_> = app.draft.as_ref()
        .map(|d| d.comments
            .iter()
            .filter(|c| {
                c.file_path.is_some()
                    && (c.github_id.is_some() || c.status != crate::review::models::CommentStatus::Rejected)
            })
            .cloned()
            .collect())
        .unwrap_or_default();
    ...
    if comments.is_empty() {
        app.show_info(
            "No fixable comments",
            "No inline comments with a file location were found.\n\
             New AI comments must not be rejected; existing GitHub comments are always fixable.",
        );
        return;
    }
